...
if(isset($_POST['btn-add-releases'])){
   ...
   if(!empty($_POST['year-of-releases']) && !empty($_POST['producer']) && !empty($_POST['name-collection']) && !empty($_POST['desc']) && !empty($_POST['info']) && !empty($_POST['number_base_cards']) && !empty($_POST['number_rookie_cards']) && !empty($_POST['number_pharallel_cards']) && !empty($sport_type) && $_FILES["img"]["name"] != ''){
     ...
     $info=mysqli_real_escape_string($con, $_POST['info']);
             ...
             $folder="../../images_new_realeses/".$newname;
    ...
    if($pop==0){
     ...
     $sql_insert="INSERT INTO realeses (sort_number, name_of_collection, year_of_releases, producer, sport_type, number_base_cards, number_rookie_cards, number_pharallel_cards, image, description, info) VALUES ($sort_number, '$name_collection', '$year_of_releases', '$producer', '$sport_type', $number_base_cards, $number_rookie_cards, $number_pharallel_cards, '$newname', '$desc', '$info')";
		
			if(mysqli_query($con, $sql_insert)){
			    echo "<h5 class='text-success'>Successfully added new realeses</h5>";
			    ?>
			    <script> 
               setTimeout(function(){
